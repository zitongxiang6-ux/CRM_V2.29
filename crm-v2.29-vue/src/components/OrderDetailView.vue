...
const basicRows = computed(() => {
  ...
  return props.admin
    ? [
        ["订单编号", o.no], ["客户名称", o.customer],
        ["项目名称", o.project], ["项目编号", o.projectNo || "-"],
        ["下单时间", o.date], ["订单金额", money(o.amount, o.currency)],
        ["应付总金额", `${payable}（订单金额：${payable}，运费：$0，手续费：$0）`], ["支付货币", o.currency],
        ["合同编号", o.contract || "-"], ["HDL Ref No", o.hdl || "-"],
        ["ERP订单号", o.erp || "-"], ["是否含税", o.tax || "-"],
        ["订单状态", statusZh[o.status] || o.status], ["付款凭证", "-"],
        ["订单备注", o.note || "-"], ["付款备注", "-"],
        ["发货备注", o.dispatch || "-"],
      ]
    : [
      ...
      <section class="page order-detail-main">
        ...
        <div class="detail-pane">
          ...
          <section v-else class="tile-pane" aria-label="Tile information">
            <div class="tile-audit-list"><div class="tile-audit-item"><span class="tile-audit-dot"></span><div class="tile-meta-grid"><span>{{ admin ? "审核状态：" : "Approval Status:" }}</span><strong>{{ admin ? "待确认" : "To be confirmed" }}</strong><span>{{ admin ? "审核说明：" : "Approval Instructions:" }}</span><strong>-</strong><span>{{ admin ? "审核时间：" : "Approval Time:" }}</span><strong>-</strong><span>{{ admin ? "审核人：" : "Reviewer:" }}</span><strong>-</strong></div></div><div class="tile-audit-item"><span class="tile-audit-dot"></span><div><div class="tile-meta-grid"><span>{{ admin ? "上传时间：" : "Upload Time:" }}</span><strong>2026/07/16</strong><span>{{ admin ? "上传人：" : "Uploader:" }}</span><strong>{{ order.owner }}</strong></div><h2 class="tile-document-title">{{ admin ? "镭雕需求文件" : "Laser Engraving Requirements Document" }}</h2><div class="tile-document-list"><div class="tile-document-row"><span class="tile-document-name">{{ admin ? "20259151615371955" : "20253131048219160" }}</span><div class="tile-document-actions"><button type="button" @click="emit('toast', admin ? '文档预览已打开。' : 'Document preview opened.')">{{ admin ? "预览" : "Preview" }}</button><button type="button" @click="emit('toast', admin ? '文档已下载。' : 'Document downloaded.')">{{ admin ? "下载" : "Download" }}</button></div></div><div v-if="!admin" class="tile-document-row"><span class="tile-document-name">销售单.xlsx</span><div class="tile-document-actions"><button type="button" @click="emit('toast', admin ? '文档预览已打开。' : 'Document preview opened.')">{{ admin ? "预览" : "Preview" }}</button><button type="button" @click="emit('toast', admin ? '文档已下载。' : 'Document downloaded.')">{{ admin ? "下载" : "Download" }}</button></div></div></div></div></div></div>
          </section>
        </div>
      </section>
