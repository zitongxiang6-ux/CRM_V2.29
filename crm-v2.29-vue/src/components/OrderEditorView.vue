...
const canEditStandardCode = computed(() => ["modify", "audit"].includes(props.mode));
...
<template>
  <form class="order-editor-view" @submit.prevent="submitEditor">
    ...
    <section class="page editor-products-card">
      ...
      <div v-else class="table-wrap editor-line-table custom-editor-table"><table><thead><tr><th>{{ isChinese ? "产品名称" : "Product Name" }}</th><th>{{ isChinese ? "产品规格" : "Product Specification" }}</th><th>{{ isChinese ? "产品编码" : "Product Code" }}</th><th>{{ isChinese ? "非标品编码" : "No-standard Product Code" }}</th><th>{{ isChinese ? "数量" : "Quantity" }}</th><th>{{ isChinese ? "单价" : "Unit Price" }}</th><th>{{ isChinese ? "是否镭雕" : "Laser Engraving" }}</th><th>{{ isChinese ? "非标定制需求" : "No-standard customization" }}</th><th>{{ isChinese ? "金额" : "Amount" }}</th><th>{{ isChinese ? "发货状态" : "Delivery Status" }}</th><th>{{ isChinese ? "操作" : "Operation" }}</th></tr></thead><tbody><tr v-for="(line, index) in customLines" :key="line.productId"><td>{{ productFor(line).name }}</td><td>{{ productFor(line).spec }}</td><td>{{ productFor(line).code }}</td><td><input v-model="line.noStandardCode" class="cell-input code" :aria-label="isChinese ? '非标品编码' : 'No-standard Product Code'" /></td><td><input v-model.number="line.qty" class="cell-input quantity" type="number" min="1" /></td><td><template v-if="canEditCustomPrice"><span class="currency-prefix">{{ order.currency === 'HKD' ? 'HK$' : '$' }}</span><input v-model.number="line.price" class="cell-input price" type="number" min="0" /></template><span v-else>{{ money(line.price, order.currency) }}</span></td><td><select v-if="canEditCustomDetails" v-model="line.laser" class="cell-select"><option>Yes</option><option>No</option></select><span v-else>{{ line.laser }}</span></td><td><input v-if="canEditCustomDetails" v-model="line.customization" class="cell-input customization" /><span v-else>{{ line.customization }}</span></td><td class="money">{{ mode === "confirm" ? "***" : money(line.price * line.qty, order.currency) }}</td><td>{{ isChinese ? "待发货" : "Pending Shipment" }}</td><td><button class="btn red small" type="button" @click="removeLine(customLines, index)"><Trash2 :size="14" />{{ isChinese ? "删除" : "Delete" }}</button></td></tr></tbody></table></div>
    </section>
